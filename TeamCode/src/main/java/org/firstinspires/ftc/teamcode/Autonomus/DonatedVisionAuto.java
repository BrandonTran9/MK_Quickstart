/*
        ...
        say "if the numeric value is 21, then run the GPP pathbuilder". Right now, though, the code just has movement. So whenever you get your shooting and intake mechanisms figured out, just add that code in the
...
@Autonomous(name = "April Tag with PP test", group = "Examples")
@Configurable // Panels
@SuppressWarnings("FieldCanBeLocal") // Stop Android Studio from bugging about variables being predefined
public class DonatedVisionAuto extends NextFTCOpMode {
    ...
    // Initialize poses
    private final Pose startPose = new Pose(8, 8, Math.toRadians(90)); // Start Pose of our robot.
    private final Pose scorePose = new Pose(35, 35, Math.toRadians(115)); // Scoring Pose of our robot. It is facing the goal at a 115 degree angle.
    private final Pose PPGPose = new Pose(8, 8, Math.toRadians(0)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose PGPPose = new Pose(36, 8, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose GPPPose = new Pose(8, 36, Math.toRadians(0)); // Lowest (Third Set) of Artifacts from the Spike Mark.
    ...
    private int foundID; // Current state machine value



    public DonatedVisionAuto() {
        addComponents(
    ...
    public void buildPathsPPG() {
        ...
        // This variable 'scorePPG' is assigned twice, the first assignment is overwritten.
        // If you intend to build a chain, you should use different variable names or chain them.
        // For now, assuming you meant to build 'grabPPG' first.
        grabPPG = follower.pathBuilder() // Changed from scorePPG to grabPPG to match state machine
    ...
    public void updateStateMachinePPG() {
        switch (pathStatePPG) {
            case 0:
                ...
                setpathStatePPG(1); // Corrected: Call the setter method
                break;
            case 1:
    ...
    public void updateStateMachinePGP() {
        switch (pathStatePGP) {
            case 0:
                ...
                setpathStatePGP(1); // Corrected: Call the setter method
                break;
            case 1:
                // Wait until we have passed all path constraints
                if (!follower.isBusy()) {
                    ...
                    setpathStatePGP(-1); // Corrected: Call the setter for PGP
                }
                break;
        }
    }
    ...
    public void updateStateMachineGPP() {
        switch (pathStateGPP) {
            case 0:
                ...
                setpathStateGPP(1); // Corrected: Call the setter method
                break;
            case 1:
                // Wait until we have passed all path constraints
                if (!follower.isBusy()) {
    ...
    // Setter methods are now correctly placed at the class level
